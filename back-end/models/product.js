...
class Product extends Model {
  ...
  async determineRole(userId) {
    ...
    if (this.get('sellerId') === userId) {
      this.role = 'Seller';
      if (this.get('state') !== 'Evaluating Offers') return;
    }
    ...
    if (this.get('state') === 'Sold' && this.get('buyerId') === userId) {
      ...
      const bids = await this.getBids({
        where: {
          bidderId: userId,
          state: ['Rejected', 'Out-bid', 'Other Bid Accepted'],
        },
      });
      ...
      const bids = await this.getBids({
        where: {
          bidderId: userId,
          state: ['Under Evaluation', 'Rejected'],
        },
        attributes: ['state'],
      });
  ...
  async acceptBid(userId) {
    ...
    if (this.get('state') === 'Active') {
      throw new BidError(
        'You must wait for the product listing to expire before accepting bids'
      );
    }

    if (this.get('state') === 'Inactive') {
      throw new BidError(
        'You cannot accept bids on an inactive product listing'
      );
    }

    if (this.get('state') === 'Sold') {
      throw new BidError(
        'This product has been sold - you already accepted a bid'
      );
    }
    ...
    if (highestBid.length) {
      ...
      await sequelize.models.Bid.update(
        {
          state:
            listingType === 'Listing Price' ? 'Other Bid Accepted' : 'Out-bid',
        },
        {
          where: {
            productId: this.get('id'),
      ...
      throw new BidError(
        'There are no more bids for your product - it is now inactive'
      );
    }
  }
  ...
  async rejectBid(userId) {
    ...
    if (this.get('state') === 'Active') {
      throw new BidError(
        'You must wait for the product listing to expire before rejecting bids'
      );
    }

    if (this.get('state') === 'Inactive') {
      throw new BidError(
        'You cannot reject bids on an inactive product listing'
      );
    }

    if (this.get('state') === 'Sold') {
      throw new BidError(
        'This product has been sold - all other bids were rejected'
      );
    }
  ...
  async createReview(userId, reviewInfo) {
    ...
    if (sellerId !== userId && buyerId !== userId) {
      throw new ReviewError(
        'You are not allowed to leave a review on this product listing'
      );
    }

    if (this.get('state') !== 'Sold') {
      throw new ReviewError(
        'You must wait for the product to be sold before leaving a review'
      );
    }
