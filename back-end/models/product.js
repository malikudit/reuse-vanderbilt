...
Product.init({
        ...
        validate: {
            ...
            verifyListingPrice(value) {
                if (value === 'Listing-Only' && !this.listingPrice) {
                    throw new Error('Listing price is a required field if you are selling at just a listing price');
                }
            },
            verifyBuyNowPrice(value) {
                if (value === 'Bid-And-Buy-Now' && !this.buyNow) {
                    throw new Error('Buy now price is a required field if you are allowing buy now with bidding');
                }
            },
            verifyBiddingFields(value) {
                ...
                if (!this.currentBid) {
                    throw new Error('Current bid price is a required field if you are supporting bidding');
                }
            }
        },
    },
    ...
    state: {
        type: DataTypes.ENUM('Active', 'Inactive', 'Evaluating Offers', 'Booked', 'Sold'),
        allowNull: false,
        validate: {
            notNull: {
